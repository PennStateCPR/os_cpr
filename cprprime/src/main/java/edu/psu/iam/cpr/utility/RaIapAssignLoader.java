...
import edu.psu.iam.cpr.utility.beans.RaIapAssign;

public class RaIapAssignLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory,
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				RaIapAssign bean = new RaIapAssign();
				...
				// registration_authority_key|ra_iap_assign_key|iap_key

				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("registration_authority_key")) {
						bean.setRegistrationAuthorityKey(new Long(fields[i]));
					}
					else if (columns[i].equals("ra_iap_assign_key")) {
						bean.setRaIapAssignKey(new Long(fields[i]));
					}
