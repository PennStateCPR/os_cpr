...
import edu.psu.iam.cpr.utility.beans.CitizenshipCs;

public class CitizenshipCsLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory, String tableName) {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				CitizenshipCs bean = new CitizenshipCs();
				...
				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("citizenship")) {
						bean.setCitizenship(new Long(fields[i]));
					}
					else if (columns[i].equals("description")) {
						bean.setDescription(fields[i]);
					}
