...
import edu.psu.iam.cpr.utility.beans.AcademicCollegeCs;

public class AcademicCollegeCsLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory, String tableName) {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				AcademicCollegeCs bean = new AcademicCollegeCs();
				...
				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("college_code")) {
						bean.setCollegeCode(fields[i]);
					}
					else if (columns[i].equals("college_name")) {
						bean.setCollegeName(fields[i]);
					}
