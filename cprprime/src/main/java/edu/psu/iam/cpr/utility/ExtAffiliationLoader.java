...
import edu.psu.iam.cpr.utility.beans.ExtAffiliation;

public class ExtAffiliationLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory,
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				ExtAffiliation bean = new ExtAffiliation();
				bean.setCreatedBy(requestor);
				bean.setCreatedOn(d);
				bean.setLastUpdateBy(requestor);
				bean.setLastUpdateOn(d);
				
				//active_flag|ext_affiliation_type_key|ext_affiliation|ext_affiliation_desc|ext_affiliation_key
				...
				for (int i = 0; i < columns.length; ++i) {
					...
					else if (columns[i].equals("ext_affiliation")) {
						bean.setExtAffiliation(fields[i]);
					}
					else if (columns[i].equals("ext_affiliation_desc")) {
						bean.setExtAffiliationDesc(fields[i]);
					}
					else if (columns[i].equals("ext_affiliation_key")) {
						bean.setExtAffiliationKey(new Long(fields[i]));
					}
				}
