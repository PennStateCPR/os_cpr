...
import edu.psu.iam.cpr.utility.beans.IapExtMapping;

public class IapExtMappingLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory,
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				IapExtMapping bean = new IapExtMapping();
				...
				// iap_ext_mapping_key|external_iap_key|mapping_url|iap_key

				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("iap_ext_mapping_key")) {
						bean.setIapExtMappingKey(new Long(fields[i]));
					}
					else if (columns[i].equals("external_iap_key")) {
						bean.setExternalIapKey(new Long(fields[i]));
					}
					else if (columns[i].equals("mapping_url")) {
						bean.setMappingUrl((fields[i].equals("NULL")) ? null : fields[i]);
					}					
