...
import edu.psu.iam.cpr.utility.beans.VisaCs;

public class VisaCsLoader implements BeanLoader {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("visa_code")) {
						bean.setVisaCode(fields[i]);
					}
					else if (columns[i].equals("visa_desc")) {
						bean.setVisaDesc(fields[i]);
					}
