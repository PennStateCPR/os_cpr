...
import edu.psu.iam.cpr.utility.beans.EmployeeStatusCs;

public class EmployeeStatusCsLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory, String tableName) {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				EmployeeStatusCs bean = new EmployeeStatusCs();
				...
				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("status_code")) {
						bean.setStatusCode(fields[i]);
					}
					else if (columns[i].equals("status_desc")) {
						bean.setStatusDesc(fields[i]);
					}
