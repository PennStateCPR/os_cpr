...
import edu.psu.iam.cpr.utility.beans.AppointmentCs;

public class AppointmentCsLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory, String tableName) {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				AppointmentCs bean = new AppointmentCs();
				...
				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("appt_code")) {
						bean.setApptCode(fields[i]);
					}
					else if (columns[i].equals("appt_desc")) {
						bean.setApptDesc(fields[i]);
					}
					else if (columns[i].equals("appt_source")) {
						bean.setApptSource(fields[i]);
					}
