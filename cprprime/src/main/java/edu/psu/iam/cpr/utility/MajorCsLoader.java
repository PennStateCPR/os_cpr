...
import edu.psu.iam.cpr.utility.beans.MajorCs;

public class MajorCsLoader implements BeanLoader {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("major_code")) {
						bean.setMajorCode(fields[i]);
					}
					else if (columns[i].equals("major_name")) {
						bean.setMajorName(fields[i]);
					}
