...
import edu.psu.iam.cpr.utility.beans.BadPrefixes;

public class BadPrefixesLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory, String tableName) {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				BadPrefixes bean = new BadPrefixes();
				
				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("char_part")) {
