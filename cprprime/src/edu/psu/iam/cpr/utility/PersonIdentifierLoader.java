...
import edu.psu.iam.cpr.utility.beans.PersonIdentifier;

public class PersonIdentifierLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory,
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				// person_id|type_key|identifier_value

				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("person_id")) {
						bean.setPersonId(new Long(fields[i]));
					}
					else if (columns[i].equals("type_key")) {
						bean.setTypeKey(new Long(fields[i]));
					}
					else if (columns[i].equals("identifier_value")) {
						bean.setIdentifierValue(fields[i]);
					}
				}
