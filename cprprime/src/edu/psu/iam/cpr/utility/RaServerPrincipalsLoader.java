...
import edu.psu.iam.cpr.utility.beans.RaServerPrincipals;

public class RaServerPrincipalsLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory,
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				RaServerPrincipals bean = new RaServerPrincipals();
				...
				// ra_server_principal_key|registration_authority_key|ra_server_principal

				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("ra_server_principal_key")) {
					...
					else if (columns[i].equals("ra_server_principal")) {
						bean.setRaServerPrincipal(fields[i]);
					}
				}
