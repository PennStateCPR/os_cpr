...
import edu.psu.iam.cpr.utility.beans.ExtAffiliationMapping;

public class ExtAffiliationMappingLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory,
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				ExtAffiliationMapping bean = new ExtAffiliationMapping();
				...
				// affiliation_key|ext_affiliation_mapping_key|ext_affiliation_key

				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("affiliation_key")) {
						bean.setAffiliationKey(new Long(fields[i]));
					}
					else if (columns[i].equals("ext_affiliation_mapping_key")) {
						bean.setExtAffiliationMappingKey(new Long(fields[i]));
					}
