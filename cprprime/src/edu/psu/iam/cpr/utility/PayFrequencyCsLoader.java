...
import edu.psu.iam.cpr.utility.beans.PayFrequencyCs;

public class PayFrequencyCsLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory, String tableName) {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				PayFrequencyCs bean = new PayFrequencyCs();
				...
				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("pay_freq_code")) {
						bean.setPayFreqCode(fields[i]);
					}
					else if (columns[i].equals("pay_freq_desc")) {
						bean.setPayFreqDesc(fields[i]);
					}
