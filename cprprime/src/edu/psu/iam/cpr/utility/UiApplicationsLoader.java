...
import edu.psu.iam.cpr.utility.beans.UiApplications;

public class UiApplicationsLoader implements BeanLoader {
...
//			ui_application_key|application_name|application_desc|suspend_flag
			while ((line = bufferedReader.readLine()) != null) {
				String[] fields = line.split("[|]");
				
				UiApplications bean = new UiApplications();
				...
				for (int i = 0; i < columns.length; ++i) {
					...
					else if (columns[i].equals("application_name")) {
						bean.setApplicationName(fields[i]);
					}
					else if (columns[i].equals("application_desc")) {
						bean.setApplicationDesc(fields[i]);
					}
