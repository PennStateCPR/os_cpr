...
import edu.psu.iam.cpr.utility.beans.IdentifierType;

public class IdentifierTypeLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory,
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				IdentifierType bean = new IdentifierType();
				bean.setCreatedBy(requestor);
				bean.setCreatedOn(d);
				bean.setLastUpdateBy(requestor);
				bean.setLastUpdateOn(d);
				
				// type_key,type_name,can_assign_flag
				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("type_key")) {
						bean.setTypeKey(new Long(fields[i]));
					}
					else if (columns[i].equals("type_name")) {
						bean.setTypeName(fields[i]);
					}
