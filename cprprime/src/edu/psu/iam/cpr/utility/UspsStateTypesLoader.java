...
import edu.psu.iam.cpr.utility.beans.UspsStateTypes;

public class UspsStateTypesLoader implements BeanLoader {
	...
	public void loadTable(Database db, String primeDirectory, String tableName) {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				// usps_state_type_key|usps_state_type

				UspsStateTypes bean = new UspsStateTypes();
				
				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("usps_state_type_key")) {
						bean.setUspsStateTypeKey(new Long(fields[i]));
					}
					else if (columns[i].equals("usps_state_type")) {
						bean.setUspsStateType(fields[i]);
					}
				}
