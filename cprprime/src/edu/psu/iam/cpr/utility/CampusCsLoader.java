...
import edu.psu.iam.cpr.utility.beans.CampusCs;

public class CampusCsLoader implements BeanLoader {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				for (int i = 0; i < columns.length; ++i) {
					if (columns[i].equals("campus")) {
						bean.setCampus(fields[i]);
					}
					else if (columns[i].equals("campus_code_key")) {
						bean.setCampusCodeKey(new Long(fields[i]));
					}
					else if (columns[i].equals("campus_code")) {
						bean.setCampusCode(fields[i]);
					}
				}
