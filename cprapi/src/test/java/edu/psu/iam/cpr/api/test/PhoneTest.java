...
public class PhoneTest {
	...
	private static final String URI = "/cprapi/v1/people/person_id:100000/phones";
	...
	public void _10testAdd() throws Exception {
		...
		nvps.add(new BasicNameValuePair("phoneType", "LOCAL_PHONE"));
		nvps.add(new BasicNameValuePair("phoneNumber", "814-555-1212"));
	...
	private void deletePhone(String phoneType) {
		try {
			for (int i = 0; i < 50; ++i) {
				DefaultHttpClient httpclient = new DefaultHttpClient();
				HttpDelete httpReq = new HttpDelete(REST_HOST + URI + "?requestedBy=cpruser&phoneType=" + phoneType + "&groupId=" + i);
				String encoding = new String(Base64.encodeBase64("cpruser:abcd1234".getBytes()));
				httpReq.setHeader("Authorization", "Basic " + encoding);
				httpReq.setHeader("Accept", "application/json");
