...
/**
 * This class provides the implementation for the Get Internal Affiliations API.
 * 
 ...
 * @lastrevision $Date$
 */
public class GetInternalAffiliationsApi extends ExtendedBaseApi {
	...
	public Object runApi(String apiName, Database db, ServiceCoreReturn serviceCoreReturn,
		...
		final PersonAffiliationTable personAffiliationTable = ValidatePersonAffiliation.validateGetAffiliationsForPersonIdParameters(
												db, personId, updatedBy, returnHistory);
		
		final AffiliationReturn[] queryResults = personAffiliationTable.getInternalAffiliationsForPersonId(db, personId);
		
		// Build the return class
		return (Object) new AffiliationServiceReturn(ReturnType.SUCCESS.index(), ApiHelper.SUCCESS_MESSAGE, queryResults, 
				queryResults.length);
