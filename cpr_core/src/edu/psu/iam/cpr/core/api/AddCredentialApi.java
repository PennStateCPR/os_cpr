...
/**
 * This class provides the implementation for the Add Credential API.
 * 
 ...
 * @lastrevision $Date$
 */
public class AddCredentialApi extends BaseApi {

	/** Contains the index for the credential type parameter. */
	private static final int CREDENTIAL_TYPE = 0;
	
	/** Contains the index for the credential data parameter. */
	...
	public JsonMessage runApi(String apiName, Database db, ServiceCoreReturn serviceCoreReturn,
		...
		final CredentialTable credentialTable = ValidateCredential.validateAddCredentialParameters(db, 
				personId, credentialType, credentialData, updatedBy);

		// Determine if the caller is authorized to make this call.
		if (checkAuthorization) {
			serviceCoreReturn.getAuthorizationService().dataActionAuthorized(db, credentialTable.getCredentialType().toString(), 
