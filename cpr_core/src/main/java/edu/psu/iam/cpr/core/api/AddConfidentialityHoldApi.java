...
/**
 * This class provides the implementation for the Add Confidentiality API.
 * 
 ...
 * @lastrevision $Date$
 */
public class AddConfidentialityHoldApi extends BaseApi {
	...
	public JsonMessage runApi(final String apiName, final Database db, final ServiceCoreReturn serviceCoreReturn,
			...
			ParseException {
        final String confidentialityType = (String) otherParameters.get(CONFIDENTIALITY_TYPE_KEY);
		...
		if (checkAuthorization) {
			serviceCoreReturn.getAuthorizationService().dataActionAuthorized(db, confidentialityTable.getConfidentialityType().toString(), 
