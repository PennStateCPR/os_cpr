...
/**
 * This class provides the implementation for the Archive Name API.
 * 
 ...
 * @lastrevision $Date$
 */
public class ArchiveNameApi extends BaseApi {
	...
	public JsonMessage runApi(final String apiName, final Database db, final ServiceCoreReturn serviceCoreReturn,
		...
		final NamesTable namesTable = ValidateName.validateArchiveNameParameters(db, personId, nameType, 
				documentType, updatedBy, nameKey);
		...
		if (checkAuthorization) {
			serviceCoreReturn.getAuthorizationService().dataActionAuthorized(db, 
					namesTable.getNameType().toString(), AccessType.ACCESS_OPERATION_WRITE.toString(), 
					updatedBy);
		}
