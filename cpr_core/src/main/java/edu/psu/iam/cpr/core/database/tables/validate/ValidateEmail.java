...
public final class ValidateEmail {
     ...
     * @throws CprException 
     */
	public static EmailAddressTable validateEmailAddressParameters(final Database db, final long personId,
			final String emailType, final String emailAddress, final String updatedBy) throws CprException {
     ...
     * @throws CprException 
     */
	public static EmailAddressTable validatePrimaryEmailParameters(final Database db, final long personId,
			final String emailType, final String updatedBy) throws CprException {
		
		return validateArchiveEmailAddressParameters(db, personId, emailType, updatedBy, null);
	}	
	 ...
	 * @throws CprException 
	 */
	public static EmailAddressTable validateGetEmailAddressParameters(final Database db, final long personId, final String requestedBy, 
			final String returnHistory, final String emailKey) throws CprException {
		...
		Long emailKeyValue = -1L;
		if (emailKey != null) {
			emailKeyValue = Long.valueOf(emailKey);
		}
		...
		emailAddressTable.setEmailKey(emailKeyValue);
	 ...
	 * @param emailAddressKey contains the email address key, only used for RESTful services.
	 * @return EmailAddressTable class.
	 * @throws CprException 
	 */
	public static EmailAddressTable validateArchiveEmailAddressParameters(final Database db, final long personId, final String emailAddressType, 
			final String updatedBy, final String emailAddressKey) throws CprException {
		
		db.getAllTableColumns(DATABASE_TABLE_NAME);

		String localEmailAddressType = null;
		Long emailAddressKeyValue = -1L;
		
		if (emailAddressKey == null) {
			localEmailAddressType = ValidateHelper.checkField(db, emailAddressType, null, "Email address type", false);
		}
		else {
			emailAddressKeyValue = Long.valueOf(emailAddressKey);
		}
		...
		EmailAddressTable emailAddressTable = new EmailAddressTable(personId, localEmailAddressType, localUpdatedBy);
		emailAddressTable.setEmailKey(emailAddressKeyValue);
		return emailAddressTable;
	}
}
