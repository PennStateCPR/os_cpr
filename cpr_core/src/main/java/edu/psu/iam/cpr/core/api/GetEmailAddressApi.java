...
public class GetEmailAddressApi extends ExtendedBaseApi {
	...
	public Object runApi(final String apiName, final Database db, final ServiceCoreReturn serviceCoreReturn,
		...
		// Validate the data.
		final EmailAddressTable emailAddressTable = ValidateEmail.validateGetEmailAddressParameters(db, personId, 
																			updatedBy, returnHistory, emailKey);
		
		// Perform the query.
		final EmailAddressReturn[] queryResults = emailAddressTable.getEmailAddressForPersonId( db, personId);

		// Build the return class.
		return new EmailAddressServiceReturn(ReturnType.SUCCESS.index(), ApiHelper.SUCCESS_MESSAGE, 
